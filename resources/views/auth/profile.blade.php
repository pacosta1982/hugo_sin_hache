            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="text-center">
                        <div class="text-3xl font-bold text-blue-600 mb-2">{{ $employee ? $employee->orders->count() : 0 }}</div>
                        <div class="text-sm text-gray-600">Canjes realizados</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold text-green-600 mb-2">{{ number_format($employee ? $employee->puntos_totales : 0) }}</div>
                        <div class="text-sm text-gray-600">Puntos totales</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold text-purple-600 mb-2">{{ $employee ? $employee->favorites->count() : 0 }}</div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-lg font-bold text-gray-900">{{ number_format($employee ? $employee->puntos_totales : 0) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm font-medium text-gray-600">Puntos canjeados:</span>
                        <span class="text-lg font-bold text-red-600">{{ number_format($employee ? $employee->puntos_canjeados : 0) }}</span>
                    </div>
                    <div class="border-t border-gray-200 pt-4">
                        <div class="flex justify-between items-center">
                            <span class="text-base font-semibold text-gray-900">Puntos disponibles:</span>
                            <span class="text-xl font-bold text-green-600">{{ number_format($employee ? ($employee->puntos_totales - $employee->puntos_canjeados) : 0) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <dl class="space-y-3">
                    <div>
                        ...
                        <dd class="text-sm text-gray-900 font-mono">{{ $employee ? $employee->id_empleado : '' }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-600">Rol de usuario:</dt>
                        <dd class="text-sm">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                {{ $employee && $employee->is_admin ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                {{ $employee && $employee->is_admin ? 'Administrador' : 'Usuario' }}
                            </span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-600">Miembro desde:</dt>
                        <dd class="text-sm text-gray-900">{{ $employee ? $employee->created_at->format('d/m/Y') : '' }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-600">Última actividad:</dt>
                        <dd class="text-sm text-gray-900">{{ $employee ? $employee->updated_at->format('d/m/Y H:i') : '' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('favorites.index') }}" class="w-full btn-ghost text-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
